...
export const ProductGrid = ({
  ...
  return <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
         {products.map(product => <Product product={product} key={product.id} />)}
      </div>;
};
export const ProductSkeletonGrid = () => {
  return <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
         {[...Array(12)].map(() => <ProductSkeleton key={Math.random()} />)}
      </div>;
};
export const Product = ({
  ...
  return <Link className="" to={`/products/${product.id}`}>
         <Card className="h-full">
            <CardHeader className="p-0">
               <div className="relative h-60 w-full">
                  <img className="rounded-t-lg" src={getProductCardThumbUrl(product?.images?.[0])} alt="product image" sizes="(min-width: 1000px) 30vw, 50vw" style={{
            objectFit: 'cover'
          }} />
               </div>
            </CardHeader>
