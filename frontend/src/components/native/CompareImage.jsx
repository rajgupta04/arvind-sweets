...
const CompareImage = props => {
  ...
  useEffect(() => {
    ...
    if (allImagesLoaded) {
      ...
      const idealWidthHeightRatio = aspectRatio === 'taller' ? Math.max(leftImageWidthHeightRatio, rightImageWidthHeightRatio) : Math.min(leftImageWidthHeightRatio, rightImageWidthHeightRatio);
  ...
  const styles = {
    ...
    rightImage: {
      clip: horizontal ? `rect(auto, auto, auto, ${containerWidth * sliderPosition}px)` : `rect(${containerHeight * sliderPosition}px, auto, auto, auto)`,
      display: 'block',
      height: '100%',
      objectFit: 'cover',
      position: 'absolute',
    ...
    leftImage: {
      clip: horizontal ? `rect(auto, ${containerWidth * sliderPosition}px, auto, auto)` : `rect(auto, auto, ${containerHeight * sliderPosition}px, auto)`,
      display: 'block',
      height: '100%',
      objectFit: 'cover',
      position: 'absolute',
  ...
  return <>
            {skeleton && !allImagesLoaded && <div style={{
      ...styles.container
    }}>{skeleton}</div>}
      ...
      display: allImagesLoaded ? 'block' : 'none'
                ...
                <img onLoad={() => setRightImgLoaded(true)} alt={rightImageAlt} data-testid="right-image" ref={rightImageRef} src={rightImage} style={styles.rightImage} />
                <img onLoad={() => setLeftImgLoaded(true)} alt={leftImageAlt} data-testid="left-image" ref={leftImageRef} src={leftImage} style={styles.leftImage} />
                <div style={styles.slider}>
                    <div style={styles.line} />
                    {handle ? <div style={styles.handleCustom}>{handle}</div> : <div style={styles.handleDefault}>
                ...
                {/* labels */}
                {leftImageLabel && <div style={styles.leftLabelContainer}>
                        <div style={styles.leftLabel}>{leftImageLabel}</div>
                    </div>}
                {rightImageLabel && <div style={styles.rightLabelContainer}>
