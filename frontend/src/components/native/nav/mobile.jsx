         ...
         <SheetContent side="left" className="pr-0">
            ...
            <ScrollArea className="my-4 h-[calc(100vh-8rem)] pb-10 pl-6">
               <div className="flex flex-col space-y-3">
                  {docsConfig.mainNav?.map(item => item.href && <MobileLink key={item.href} href={item.href} onOpenChange={setOpen}>
                              {item.title}
                           </MobileLink>)}
               </div>
               <div className="flex flex-col space-y-2">
                  {docsConfig.sidebarNav.map((item, index) => <div key={index} className="flex flex-col space-y-3 pt-6">
